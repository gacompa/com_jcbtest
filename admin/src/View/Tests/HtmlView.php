...
/------------------------------------------------------------------------------------------------------*/
namespace JCB\Component\Jcbtest\Administrator\View\Tests;
...
/**
 * Jcbtest Html View class for the Tests
 *
 * @since  1.6
 */
...
class HtmlView extends BaseHtmlView
{
	...
	public function display($tpl = null): void
	{
		...
		$this->canBatch = ($this->canDo->get('test.batch') && $this->canDo->get('core.batch'));
	...
	{
		ToolbarHelper::title(Text::_('COM_JCBTEST_TESTS'), 'joomla');

		if ($this->canCreate)
		{
			ToolbarHelper::addNew('test.add');
		}
		...
		if (ArrayHelper::check($this->items))
		{
			if ($this->canEdit)
			{
				ToolbarHelper::editList('test.edit');
			}

			if ($this->canState)
			{
				ToolbarHelper::publishList('tests.publish');
				ToolbarHelper::unpublishList('tests.unpublish');
				ToolbarHelper::archiveList('tests.archive');

				if ($this->canDo->get('core.admin'))
				{
					ToolbarHelper::checkin('tests.checkin');
				}
			}

			if ($this->state->get('filter.published') == -2 && ($this->canState && $this->canDelete))
			{
				ToolbarHelper::deleteList('', 'tests.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				ToolbarHelper::trash('tests.trash');
			}
		}

		// set help url for this view if found
		$this->help_url = JcbtestHelper::getHelpUrl('tests');
	...
	protected function _prepareDocument(): void
	{
		...
		$this->getDocument()->setTitle(Text::_('COM_JCBTEST_TESTS'));
		// add styles
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.group' => Text::_('COM_JCBTEST_TEST_GROUP_LABEL'),
