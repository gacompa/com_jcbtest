 ...
 * @since  1.6
 */
class TestsModel extends ListModel
{
	/**
	...
	protected array $styles = [
		...
		'administrator/components/com_jcbtest/assets/css/tests.css'
 	];
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		if ($formSubmited)
		{
			$group = $app->input->post->get('group');
	...
	protected function getListQuery()
	{
		...
		// From the jcbtest_item table
		$query->from($db->quoteName('#__jcbtest_test', 'a'));
		...
		if (!$user->authorise('core.options', 'com_jcbtest'))
		{
	...
	*/
	public function getExImPortHeaders()
	{
		...
		$db = Factory::getDbo();
		// get the columns
		$columns = $db->getTableColumns("#__jcbtest_test");
	...
	protected function checkInNow(): bool
	{
		...
		$time = ComponentHelper::getParams('com_jcbtest')->get('check_in');
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__jcbtest_test'));
			...
			if ($db->getNumRows())
			{
				...
				// Check table.
				$query->update($db->quoteName('#__jcbtest_test'))->set($fields)->where($conditions); 
