...
/------------------------------------------------------------------------------------------------------*/
namespace JCB\Component\Jcbtest\Administrator\Table;
 ...
 * @since  1.6
 */
class TestTable extends Table implements VersionableTableInterface, TaggableTableInterface, CurrentUserInterface
{
	...
	function __construct(DatabaseDriver $db, ?DispatcherInterface $dispatcher = null)
	{
		...
		$this->typeAlias = 'com_jcbtest.test';
		...
		parent::__construct('#__jcbtest_test', 'id', $db, $dispatcher);
	}
	...
	/**
	 * Overload the store method for the Test table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New test. A test created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			{
				$this->setError(Text::_('COM_JCBTEST_TEST_ERROR_UNIQUE_ALIAS'));

				if ($table->published === -2)
				{
					$this->setError(Text::_('COM_JCBTEST_TEST_ERROR_UNIQUE_ALIAS_TRASHED'));
				}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_jcbtest.test.'.$this->id);
			$this->setRules($rules);
		}
	...
	protected function _getAssetParentId(Table $table = null, $id = null)
	{
		...
		// load the jcbtest asset
		$assets->loadByName('com_jcbtest');
